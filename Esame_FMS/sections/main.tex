...
\begin{frame}{\wzjj $\,$ differential measurements}
    ...
    Differential measurements of \wzjj$\,$ production cross-section are performed as a function of three variables sensitive to anomalies in the quartic gauge coupling in \wzjj$\,$ events:
    
    \vspace{0.4cm}
    
    \begin{description}
        \setlength{\itemsep}{0.4cm} % Gestisce la spaziatura di tutto l'elenco in automatico
        
        \item[$\mathbf{\sum p_\text{T}^\ell}$] the scalar sum of the transverse momenta of the three charged leptons associated with $W$ and $Z$ bosons;
        
        \item[$\mathbf{\Delta\phi(W,Z)}$] the difference in azimuthal angle between the $W$ and $Z$ bosons' directions; % controllare come è definita
        
        \item[$\mathbf{m_\text{T}^{WZ}}$] the transverse mass of the $W^\pm Z$ system.
    \end{description}
...
\begin{frame}{\wzjj $\,$ differential measurements}
    ...
    \begin{figure}[t]
        \centering
        \subfloat[]{
            \includegraphics[width=0.31\textwidth]{Immagini/fig_07a.pdf}
        }
        \subfloat[]{
            \includegraphics[width=0.31\textwidth]{Immagini/fig_07b.pdf}
        }
        \subfloat[]{
            \includegraphics[width=0.31\textwidth]{Immagini/fig_07c.pdf}
        }
    \end{figure}
...
\begin{frame}{Search for aQGC: Methodology}
    {\small Anomalous Quartic Gauge Couplings (aQGC) modeled via EFT dimension-8 operators drastically enhance the cross-section at high energy scales ($\hat{s}$).}
    
    \smallskip
    \textbf{The 2D Fit Strategy}
    {\small
    \begin{itemize}
        \item \textbf{Observables:} The search exploits a 2D distribution combining the BDT score (to separate EWK from QCD) and the transverse mass $m_T^{WZ}$ (sensitive to $\hat{s}$).
        \item \textbf{Binning:} 4 Bins in BDT score $\times$ 5 bins in $m_T^{WZ}$ $\rightarrow$ Unrolled into a 1D histogram of 20 statistically independent bins.
        \item \textbf{Test Statistic:} Profiled Likelihood Ratio $q(\vec{c})$ to extract 95\% Confidence Level (CL) limits on the Wilson coefficients $f_j^{(8)}/\Lambda^4$.
    \end{itemize}
    }
...
\begin{frame}{Search for aQGC: Methodology}
    ...
    {\small Anomalous Quartic Gauge Couplings (aQGC) modeled via \textbf{EFT dimension-8 operators} drastically enhance the cross-section at high energy scales ($\hat{s}$).}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \includegraphics[width=0.95\textwidth]{Immagini/fig_11.pdf}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{center}
                ...
                \includegraphics[width=0.98\textwidth]{Immagini/fig_12.pdf}
            \end{center}
        \end{column}
...
\begin{frame}{Tree-Level Unitarity Violation and Clipping}
    \textbf{The Unitarity Bound}
    
    {\small The EFT is not a complete model. Non-zero dimension-8 operators violate tree-level unitarity at high energies.}
    
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \textbf{The "Clipping" Technique}
            {\footnotesize % Ridotto da \small a \footnotesize per salvare spazio
            \begin{itemize}
        ...
        \begin{column}{0.5\textwidth}
            \begin{center}
                ...
                \includegraphics[width=0.83\textwidth]{Immagini/fig_13a.pdf}
                
                \smallskip
                {\scriptsize Evolution of 95\% CL limits vs cut-off scale\par}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Conclusions: VBS as a Precision Probe for the EWK Sector}
    ...
    \begin{block}{From Discovery to Precision Measurement (Run 2, 140 fb$^{-1}$)}
        \begin{itemize}
            ...
            \item \textbf{Differential measurements} (via Bayesian Unfolding) on aQGC-sensitive variables: $m_{jj}, \Delta\phi, m_T^{WZ}, \sum p_T^\ell$.
        \end{itemize}
    \end{block}
    
    \begin{block}{Standard Model Verification and Modelling Challenges}
        \begin{itemize}
            \item \textbf{EWK Production:} Excellent Data/Theory agreement in the Signal Region.
