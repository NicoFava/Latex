...
\begin{frame}{Search for aQGC: Methodology}
    ...
    \vfill % Spinge il blocco seguente verso il fondo in modo bilanciato
    
    \begin{block}{The Matrix Element}
    \small
...
\begin{frame}{EFT Limits (Without Unitarization)}
    \begin{columns}[T]
        ...
        \begin{column}{0.45\textwidth}
            ...
            \smallskip
            {\scriptsize 
            \renewcommand{\arraystretch}{1.15} % Spazio tra le righe ridotto da 1.3 a 1.15
            \begin{table}
                \begin{tabular}{lcc}
                \hline
                \textbf{Coupling} & \textbf{Obs. [TeV$^{-4}$]} & \textbf{Exp. [TeV$^{-4}$]} \\
                \hline
                $f_{T0}/\Lambda^4$ & $[-0.57, 0.56]$ & $[-0.80, 0.80]$ \\
                $f_{T1}/\Lambda^4$ & $[-0.39, 0.35]$ & $[-0.52, 0.49]$ \\
                $f_{T2}/\Lambda^4$ & $[-1.2, 1.0]$ & $[-1.6, 1.4]$ \\
                $f_{M0}/\Lambda^4$ & $[-5.8, 5.6]$ & $[-8.3, 8.3]$ \\
                \hline
                \end{tabular}
            \end{table}
            }
            ...
            {\small
            \begin{itemize}
                \item Limits extracted setting one coefficient non-zero at a time.
            \end{itemize}
            }
        \end{column}
        ...
        \begin{column}{0.55\textwidth}
            \begin{center}
                ...
                \includegraphics[width=0.9\textwidth,height=0.6\textheight,keepaspectratio]{Immagini/fig_12.pdf}
            \end{center}
        \end{column}
...
\begin{frame}{Tree-Level Unitarity Violation and Clipping}
    ...
    {\small The EFT is not a UV-complete model. Non-zero dimension-8 operators violate tree-level unitarity at high energies.}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Limits degrade (widen) as the cut-off $E_c$ becomes more stringent (lower values).
            \end{itemize}
            }
        ...
        \begin{column}{0.5\textwidth}
            \begin{center}
                ...
                \includegraphics[width=0.95\textwidth,height=0.55\textheight,keepaspectratio]{Immagini/fig_13a.pdf}
...
\begin{frame}{Conclusions: VBS as a Precision Probe for the EWK Sector}
    ...
    \begin{block}{Limits on New Physics (EFT) and Unitarity}
        \begin{itemize}
            \item No high-energy kinematic excesses observed.
            \item \textbf{95\% CL limits} set on 9 dimension-8 operators (via Profiled Likelihood Ratio).
            \item Limits unitarized via theoretical \textbf{clipping} to prevent tree-level unitarity violation.
        \end{itemize}
    \end{block}
