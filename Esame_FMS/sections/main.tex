...
\begin{frame}{Irreducible Backgrounds}
    \textbf{Constrained via Control Regions (ZZ-CR \& b-CR):}
    \begin{itemize}
...
\begin{frame}{Signal Extraction: Multivariate Analysis (BDT)}
    ...
    \begin{itemize}
        ...
        \item \textit{Mixed Correlations:} $\ell$-centrality ($\zeta_{lep}$), event balance.
    \end{itemize}
    \textbf{BDT Output:}
    \begin{itemize}
    ...
    \end{itemize}
    \vspace{0.2cm}
    \textit{Note: A different BDT is also used to differential the background in the b-CR between $t\bar{t}V$ $tZj$.}
\end{frame}
...
\begin{frame}{\wzjj $\,$ differential measurements}
    Differential mesurements of \wzjj$\,$ production cross-section are performed as a function of three variables sensitive to anomalies in the quartic gauge coupling in \wzjj$\,$ events:
    
    \vspace{0.3cm}
    \begin{itemize}
        \item the scalar sum of the trasverse momenta of the three charged leptons associated with $W$ and $Z$ bosons $\sum p_\text{T}^\ell$;
        \vspace{0.2cm}
        \item the difference in azimuthal angle $\Delta\phi(W,Z)$ between the $W$ and $Z$ bosons' directions;
        %controllre come è definita
        \vspace{0.2cm}
        \item the trasverse mass of the $W^\pm Z$ system $m_\text{T}^{WZ}$.
    \end{itemize}
\end{frame}
\begin{frame}{\wzjj $\,$ differential measurements}
    \setcounter{subfigure}{0}
...
\begin{frame}{2D BDT vs $m_T^{WZ}$ Distribution for EFT Search}    
\begin{center}                
    \includegraphics[width=0.55\textwidth]{Immagini/fig_11.pdf}
    \end{center}
\end{frame}
